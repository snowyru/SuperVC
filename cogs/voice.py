...
class voice(commands.Cog):
    ...
    async def setup(self, ctx):
        ...
        id = ctx.author.id
        if ctx.author.id == ctx.guild.owner_id or ctx.author.id == 151028268856770560 or any(role.name == 'admin' for role in ctx.author.roles):
            def check(m):
    ...
    async def setlimit(self, ctx, num):
        conn = sqlite3.connect('voice.db')
        c = conn.cursor()
        if ctx.author.id == ctx.guild.owner.id or ctx.author.id == 151028268856770560 or any(role.name == 'admin' for role in ctx.author.roles): 
